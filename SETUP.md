...
2. **① 검색 결과 (Table 1)**: 검색 결과를 #, wfco, ingrnd_t, kingrnd_t, cfno 컬럼으로 표시.
   행을 클릭하면 해당 wfco의 **앞 6자리**로 Table 2를 생성하고 자동으로 스크롤합니다.
3. **② 동일성분 목록 (Table 2)**: wfco 앞 6자리가 같은 성분 목록.
   행을 클릭하면 **wfco 전체**로 Table 3을 생성하고 자동으로 스크롤합니다.
4. **③ 의약품 목록 (Table 3)**: `medicines_medicine`에서 wfco가 일치하는 의약품 목록.
